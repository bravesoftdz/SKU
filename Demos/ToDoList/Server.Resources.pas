...
uses
    StrUtils
  , Skuchain.Core.Registry, Skuchain.Rtti.Utils
...
begin
  ...
  if LCommand.RowsAffected < 1 then
    raise ESkuchainHttpException.CreateFmt('Item %d not found or access denied', [id], 404);
end;
...
begin
  ...
  LQuery.Open;
  if LQuery.IsEmpty then
    raise ESkuchainHttpException.CreateFmt('Item %d not found or access denied', [AItem.Id], 404);
  ...
  FD.Query('select * from ACCOUNT where ID = :Token_Claim_ACCOUNT_ID', nil, nil,
    ...
    begin
      if AQuery.IsEmpty then
        raise ESkuchainHttpException.Create('Account not found or access denied', 404);
...
initialization
  TSkuchainResourceRegistry.Instance.RegisterResource<TItemResource>;
  TSkuchainResourceRegistry.Instance.RegisterResource<TAccountResource>;
